...
const Master = ({ samples, chordProg, padSound, numOfSteps = 16, drumTracks }: MasterProps) => {
  ...
  // For the drums:
  const trackIds: number[] = [...Array(samples?.sounds?.length).keys()];
  ...
  const howlerSampler = {
    ...
    playChord() {
      ...
      nextChord.intervals
        .map((interval:string) => transpose(nextChordRoot, interval))
        .forEach((chordNote:NoteLiteral) => {
  ...
  useEffect(() => {
    ...
    tracksRef.current = samples?.sounds?.map((sample:SampleType, i:number) => ({
      id: i,
    ...
    return () => {
      ...
      tracksRef.current?.map((tr:TrType) => tr.sampler.dispose());
    }
  ...
  const muteTrack = (e:React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (sampler.volume.value < 0) {
      ...
      e.currentTarget.classList.remove('ring-rose-400');
      e.currentTarget.classList.remove('shadow-rose-500/50');
      e.currentTarget.classList.remove('hover:bg-rose-300');
      e.currentTarget.classList.remove('text-rose-300');
      e.currentTarget.classList.add('ring-emerald-400');
      e.currentTarget.classList.add('shadow-emerald-500/50');
      e.currentTarget.classList.add('hover:bg-emerald-300');
      e.currentTarget.classList.add('text-emerald-100');

    } else {
      sampler.volume.value = -64;
      e.currentTarget.classList.add('ring-rose-400');
      e.currentTarget.classList.add('shadow-rose-500/50');
      e.currentTarget.classList.add('text-rose-300');
      e.currentTarget.classList.add('hover:bg-rose-300');
    }
  }
